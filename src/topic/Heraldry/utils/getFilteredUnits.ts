...
export const getFilteredUnits = ({
  ...
  const isCustomFilterActive =
    customFilter && customFilter.isActive && Array.isArray(customFilter.result);
  ...
  const filteredUnits = unitsForMapAll.filter((unit) => {
    ...
    if (
      shouldHideMissingImages &&
      brokenHashes.some((brokenHash) => unit.imageHash === brokenHash)
    ) {
    ...
    if (isTypeFilterActive) {
      ...
      const isOneOfPickedTypes = typeFilters.some((active) =>
        (unit?.type || []).includes(active)
      );
    ...
    if (shouldIgnoreFormer) {
      const areAllTypesFormer =
        (unit?.type?.length || 0) > 0 &&
        unit.type?.every((item) => item.startsWith("former"));
    ...
    if (isColorFilterActive) {
      const colorsByNames = detailsForUnitsById?.[unit.id]?.colors?.byNames || {};

      if (Object.keys(colorsByNames).length === 0) {
        return filterResponse.notMatches;
      }

      if (
        filterOperator === "or" &&
        !Object.keys(colorsByNames).some((name) =>
      ...
      if (
        filterOperator === "and" &&
        !colorFilters.every((active) =>
          Object.keys(colorsByNames).some((name) => name === active)
        )
      ) {
        return filterResponse.notMatches;
      }
    ...
    if (isAnimalFilterActive) {
      ...
      if ([WITH_ANIMAL, WITHOUT_ANIMAL].includes(animalFilters[0])) {
        ...
        const needOneAndMissing =
          filterOperator === "or" &&
          !animalFilters.some((active) => animals.includes(active));
        if (needOneAndMissing) {
          return filterResponse.notMatches;
        }

        const needAllAndMissing =
          filterOperator === "and" &&
          !animalFilters.every((active) => animals.includes(active));
    ...
    if (isItemFilterActive) {
      ...
      const needOneAndMissing =
        filterOperator === "or" &&
        !itemFilters.some((active) => items.includes(active));
      if (needOneAndMissing) {
        return filterResponse.notMatches;
      }
      ...
      const needAllAndMissing =
        filterOperator === "and" &&
        !itemFilters.every((active) => items.includes(active));
      if (needAllAndMissing) {
        return filterResponse.notMatches;
      }
    }
  ...
  const unitsForMap = filteredUnits.filter(
    (unit) =>
      typeof unit?.place?.coordinates?.lon === "number" &&
      (unit?.imagesList?.length || 0) > 0
  );
  ...
  if (customFilter && customFilter.isActive) {
    if ((customFilter?.include || []).length > 0) {
      subtitleParts.push({
        operator: "and",
        labels: [`+${(customFilter?.include || []).length} ⛨`],
      });
    }
    ...
    if ((customFilter?.exclude || []).length > 0) {
      subtitleParts.push({
        operator: "and",
        labels: [`-${(customFilter?.exclude || []).length} ⛨`],
      });
    }

    if (
      (customFilter?.phrases || []).length > 0 &&
      Array.isArray(customFilter?.result)
    ) {
      ...
      subtitleParts.push({
        operator: "or",
        labels:
          collapsePhrases(customFilter?.phrases || [], -7).map(
            (value) => `„${value}”`
          ) || [],
      });
    }
  ...
  if (shouldIgnoreFormer) {
    subtitleParts.push({
      operator: "or",
      labels: [`heraldry.unit.type.currentlyExisting`],
    });
  }
  ...
  if (typeFilters.length > 0) {
    ...
    subtitleParts.push({
      operator: "or",
      labels: typeFilters.map(
        (value) => `heraldry.unit.type.${country}.${value}`
      ),
    });
  }
  ...
  if (colorFilters.length > 0) {
    subtitleParts.push({
      ...
      labels: colorFilters.map((value) => `heraldry.color.${value}`),
    });
  }
  ...
  if (animalFilters.length > 0) {
    subtitleParts.push({
      operator: filterOperator,
      labels: animalFilters.map((value) => `heraldry.animal.${value}`),
    });
  }
  ...
  if (itemFilters.length > 0) {
    subtitleParts.push({
      operator: filterOperator,
      labels: itemFilters.map((value) => `heraldry.item.${value}`),
    });
  }
