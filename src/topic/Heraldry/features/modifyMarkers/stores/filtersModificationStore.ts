...
export const useFilterModificationStore =
  create<FiltersModificationStoreState>()(
    devtools(
      persist(() => emptyState as FiltersModificationStoreState, {
...
export const resetModifications = () =>
  useFilterModificationStore.setState(emptyState);

const getListModifier =
  (action: "reset" | "include" | "exclude") =>
  (unit: CoatOfArmsMapData, markerType: MarkerType, item: string) => {
    ...
    const ruleToAdd: ComplexManualMarker = {
      imageHash: imageHashToChange,
      note: unit.title,
    };
    ...
    useFilterModificationStore.setState((state) => {
      ...
      const includeWithRemoved = include.filter(
        ({ imageHash }) => imageHash !== imageHashToChange
      );
      const excludeWithRemoved = exclude.filter(
        ({ imageHash }) => imageHash !== imageHashToChange
      );
      ...
      localStorage.setItem(
        `${country}_maps_${markerType}_${item}`,
...
const getListForUnit =
  (action: "include" | "exclude") =>
  (unit: CoatOfArmsMapData, markerType: MarkerType) =>
  (state: FiltersModificationStoreState) => {
    const imageHashToChange = unit.imageHash;
    ...
    const list = Object.entries(markerTypeList)
      .filter(([_item, rules]) =>
        (rules[action] || []).some(
          ({ imageHash }) => imageHash === imageHashToChange
        )
...
export const selectShortcuts = (
  ...
  const animal: Shortcut[] = Object.entries(state.animal).map(
    ([name, { include = [] } = {}]) => ({
      name,
      type: "animal",
      total: include.length,
    })
  );
  const item: Shortcut[] = Object.entries(state.item).map(
    ([name, { include = [] } = {}]) => ({
      name,
      type: "item",
      total: include.length,
