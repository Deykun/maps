...
const imagesPowiaty = fs.readdirSync('./public/images/heraldry/pl/web/powiaty').map(
  (imageFile) => ({ imageSrc: `./public/images/heraldry/pl/web/powiaty/${imageFile}`, imageFile, type: 'powiaty' }),
);
...
chunks.forEach((chunk, index) => {
  const imageSrcs = chunk.map(({ imageSrc }) => imageSrc);
  console.log(imageSrcs);
  joinImages(imageSrcs).then((sprite) => {
    // Save image as file
    sprite.toFile(`./public/images/heraldry/pl/web/sprites/powiaty-${index}.png`);
  });
})
