...
export const download = async (
  ...
  if (format === "svg") {
    ...
    const isBroken =
      text.match(/width="([0-9.]+)(pt|mm)"/) ||
      text.match(/height="([0-9.]+)(pt|mm)"/);
    if (isBroken) {
      ...
      console.log(
        chalk.yellow(
          `  - SVG with not web units detected (pt, mm) - fix applied:`
        )
      );
      ...
      blob = new Blob(
        [
          text
            .replace(/width="([0-9.]+)(pt|mm)"/, "")
            .replace(/height="([0-9.]+)(pt|mm)"/, ""),
        ],
        {
          type: "image/svg+xml",
        }
  ...
  await sharp(bos)
    ...
    .toFile(`./public/images/heraldry/${lang}/${path}/${fileName}-80w.webp`)
    .catch((error) => {
      console.log(
        chalk.bgRedBright(
          [
  ...
  await sharp(bos)
    ...
    .toFile(`./public/images/heraldry/${lang}/${path}/${fileName}-320w.webp`)
    .catch((error) => {
      console.log(
        chalk.bgRedBright(
          [
        ...
        () =>
          new Promise((resolve) => {
            const fetchAndProcess = async () => {
              ...
              const format =
                unit.image?.source?.split(".")?.at(-1)?.toLowerCase() || "png";
              ...
              if (unit.image?.source) {
                if (!existsSync(expectedFilePath)) {
                  ...
                  console.log(
                    [
                      ...
                      chalk.gray(
                        `(${format} - index: ${chalk.white(unit.index)})`
                      ),
                    ].join(" ")
                  );
                } else {
                ...
                const { imagesList } = getCompressedImageSrc(
                  `images/heraldry/${country}/${path}/${fileName}.webp`,
                  path
                );

                if (
                  !unit.place?.coordinates?.lat ||
                  !unit.place?.coordinates?.lon
                ) {
                  console.log(
                    `${chalk.yellow(unit.title)} doesn't have the ${chalk.red(
                      "location"
                    )}.`
                  );
                }
                ...
                [
                  chalk.yellow(
                    `  Progress ${chalk.green(
                      `${((global.processed[path] / total) * 100).toFixed(1)}%`
                    )}.`
                  ),
                  `${chalk.white(global.processed[path])} out of ${chalk.white(
                    total
                  )}.`,
                  getTimeStatus(),
                ].join(" ")
