...
export const getDetails = async ({
  ...
  console.log(" ");
  console.log(
    `⛊  ${chalk.blue(total)} coats to fetch (${chalk.yellow(path)}).`
  );
  console.log(" ");
  ...
  for (let i = 0; i < total; i++) {
    ...
    if (existsSync(expectedFilePath)) {
      ...
      const colors = { byNames: {}, byNamesRejected: {} };
      if (alreadyFetchedDetailsById?.[unit.id]?.colors?.byNames) {
        colors.byNames =
          alreadyFetchedDetailsById[unit.id]?.colors?.byNames || {};
        colors.byNamesRejected =
          alreadyFetchedDetailsById[unit.id]?.colors?.byNamesRejected || {};
        ...
        try {
          ...
          const colorsData = await getImageColors(image);

          const { byNames, byNamesRejected } = Object.entries(
            colorsData
          ).reduce(
            (stack, [name, percentage]) => {
              if (percentage >= 2.5) {
                stack.byNames[name] = percentage;
              } else {
                stack.byNamesRejected[name] = percentage;
              }
          ...
          colors.byNames = byNames;
          colors.byNamesRejected = byNamesRejected;
        } catch (error) {
